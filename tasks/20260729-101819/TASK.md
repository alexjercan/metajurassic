...
- [ ] Support a seed query param on the practice page, and decide whether daily honors it only in dev builds; record the choice in `DECISION.md`.
- [ ] Key seeded game state in storage so it does not clobber the real daily state.
- [ ] Guard share text so seeded/dev rounds do not masquerade as the daily puzzle.
- [ ] Document the param in AGENTS.md and the README dev section.
- [ ] Use it to script one runnable example: a short "play a fixed round" walkthrough usable by E2E fixtures and manual playtests.
