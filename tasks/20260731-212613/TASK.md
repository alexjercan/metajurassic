...
- [ ] Rebuild the parser rig (method: `tasks/20260731-212557/NOTES.md`
      ...
      figures 889/10/18.
- [ ] Read the loader family as one unit: `jsonLoader.ts`, `markdownLoader.ts`,
...
- [ ] Write `DECISION.md` before moving or cutting anything, settling two
...
- [ ] Compact the comments in `src/practiceSession.ts`. The four rules
...
- [ ] Compact `src/practice.ts` the same way. `practice.ts:35-36` is already a
...
- [ ] Fix `src/frontMatter.ts:1-14` per the `DECISION.md` choice: state the true
...
- [ ] Sweep the docs in both polarities: grep for every path this task touches,
      ...
      sibling -212611).
- [ ] Re-run the rig, fill the before/after table, then `npm run ci` and
      `npm run build` inside `nix develop`.
- [ ] Confirm the five listed suites are untouched and green, and that
