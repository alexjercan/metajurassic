// Where the drawn tree comes to rest: how far it is scaled to fit the arena,
// which node the arena centres on, and when a viewport change re-runs both.
//
// The arithmetic lives in `treeLayout.ts` and is kept DOM-free so it can be
// unit-tested; this file measures the real elements, calls it, and writes the
 ...
 *
 * tasks/20260729-092339/DECISION.md fork 2 settles the ANCHOR (the target's
 * node, not the newest guess); it rejected the union, which was adopted later
 * in that same task and is recorded nowhere else.
 */
function focusRect(
    canvas: HTMLElement,
    origin: { left: number; top: number },
...
/**
 * Hand a freshly rendered tree to the scroll machinery: remember it for later
 * relayouts, start listening for viewport changes, and lay it out once the
 * canvas has painted.
 */
export function mountTreeScroll(container: HTMLElement): void {
